...
const SCORE_BEHAVIORS = [
  { key:'b1', name:'1：在系統上完成報名（出席、候補或請假皆算）', auto:'系統自動', def:1 },
  { key:'b2', name:'2：出席 1 天的幫戰（雙周賽也算 1 天）',        auto:'系統自動（依比賽紀錄）', def:1 },
  { key:'b3', name:'3：比賽全程開語音有效溝通',                    auto:'管理端手動', def:3 },
  { key:'b4', name:'4：上傳比賽個人視角影片（影片有誤可手動扣分）', auto:'系統自動（依影片上傳）', def:2 },
  { key:'b5', name:'5：戰後復盤參與',                                  auto:'管理端手動', def:2 },
];
...
async function saveScoreCfgData(cfg){
  cfg.updatedAt = Date.now();
  S.s(S.k('score_cfg'), cfg);
  if (typeof syncWriteNowFast === 'function') await syncWriteNowFast();
  else if (typeof syncWrite === 'function') syncWrite();
}

// ─── 手動加/扣分紀錄 ─────────────────────────────────────
function scoreLog(){ return S.g(S.k('score_log'), []) || []; }
async function addScoreLogEntry(entry){
  const list = scoreLog();
  list.push({ id: uid(), ts: Date.now(), updatedAt: Date.now(), ...entry });
  S.s(S.k('score_log'), list);
  // 立即同步（不只背景防抖），確保成員端很快就能看到最新積分，避免管理端已加分、成員端還是舊分數
  if (typeof syncWriteNowFast === 'function') await syncWriteNowFast();
  else if (typeof syncWrite === 'function') syncWrite();
}
async function voidScoreLogEntry(id){
...
async function confirmManualScore(){
  ...
  toast('✅ 已記錄 '+(pts>0?'+':'')+pts+' 分並同步（成員端幾秒後即可看到）','ok');
  // 重新整理出席明細畫面
  if (typeof openAttendanceDetail==='function') openAttendanceDetail(_msTarget.name);
}
async function removeManualScore(id, name){
  if(!confirm('確定作廢這筆手動積分？')) return;
  toast('作廢中，同步中...','');
  await voidScoreLogEntry(id);
  toast('已作廢並同步','ok');
