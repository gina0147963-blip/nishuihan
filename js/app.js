...
// 在最初的模式選擇畫面顯示目前版本號（小灰字），方便在手機上（沒有開發者工具可查）
// 直接肉眼比對「這台裝置」跟「剛部署的版本」是否一致，不用連 console 都打不開。
document.addEventListener('DOMContentLoaded', ()=>{
  const el=document.getElementById('build-version-tag');
  if(el){ const v=_currentBuildVersion(); el.textContent = v ? ('版本 '+v) : ''; }
});
