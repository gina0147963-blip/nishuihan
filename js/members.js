...
function _attRate(name){
  ...
  // 出場次數：只統計「報名管理有建立場次」當天的比賽（沒報名但上場也照算）；
...
function openAttendanceDetail(name){
  ...
  document.getElementById('att-detail-summary').innerHTML = r.plan
    ? `出席率：<strong style="color:${r.rate>=80?'var(--ok)':r.rate>=50?'var(--gold)':'var(--bad)'}">${r.rate}%</strong>　（有報名出席 ${r.plan} 場中，實際出賽 ${r.act} 場）<br><span style="font-size:12px;color:var(--txt2)">總出場次數：${r.totalPlayed} 場（含未報名卻上場的場次，不計入出席率；僅統計報名管理有建立場次的日期，更早的比賽紀錄為歷史備份不列入）</span>`
    : `此成員尚無「報名出席」的場次，暫無出席率資料<br><span style="font-size:12px;color:var(--txt2)">總出場次數：${r.totalPlayed} 場（僅統計報名管理有建立場次的日期）</span>`;
  // 每場活動顯示三項狀態：報名／排表／出賽
...
function _memberRowsHtml(filtered, actMap, qa){
  const tbody=filtered.map((m,idx)=>{
    ...
    return `<tr${inactive?' style="opacity:.55"':''}>
      ...
      <td>${(function(){
        ...
        const sub=a.plan===0
          ? '尚無「已報名出席且有比賽紀錄」的場次'
          : '已報名出席且有紀錄的場次中，實際出賽 '+a.act+'／應出賽 '+a.plan;
        return `<button class="btn btn-outline xs" style="color:${c};font-weight:700" onclick="openAttendanceDetail('${m.name.replace(/'/g,"\\'")}')">${a.act}/${a.plan}${a.rate===null?'':' ('+a.rate+'%)'}</button><br><span style="font-size:10px;color:var(--txt3)">${sub}</span>`;
      })()}</td>
      <td style="font-size:10px;color:var(--txt3);white-space:nowrap">出賽${a.played}場<br>回覆${a.resp}次</td>
  ...
  return tbody||'<tr><td colspan="7" style="text-align:center;color:var(--txt3);padding:24px">'+(qa==='never'?'沒有「從未出賽/報名」的成員 🎉':'尚無成員')+'</td></tr>';
}

function renderAdminMembers(pane){
  const v=_memberView();
  pane.innerHTML=`<div class="sec-head">
    <h2>👥 成員管理 <span id="ms-count" style="font-size:12px;color:var(--txt2);font-weight:400">（${ORG_LABEL()}專屬，${v.members.length} 人${(x=>x?'・暫離 '+x+' 人':'')(v.members.filter(m=>m.status==='暫離').length)}${v.qa?'，顯示 '+v.filtered.length+' 人':''}）</span></h2>
  ...
  <div class="tbl-wrap"><table>
    <thead><tr><th style="width:36px"></th><th>成員名稱</th><th>職業</th><th>出席場數</th><th>出賽/回覆</th><th>絕技</th><th>操作</th></tr></thead>
...
function filterMembers(){
  ...
  if(cnt) cnt.textContent='（'+ORG_LABEL()+'專屬，'+v.members.length+' 人'+(awayN?'・暫離 '+awayN+' 人':'')+((v.qa||v.qs||v.qc)?'，顯示 '+v.filtered.length+' 人':'')+'）';
}
