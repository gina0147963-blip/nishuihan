...
// 計算成員出席率：實際出賽場數 ÷ 被排進排表系統（含候補）的場數（含別名），確保不會超過100%
function _attRate(name){
  const me=S.members().find(x=>x.name===name);
  const alias=new Set([name,...((me&&me.aliases)||[])]);
  const events=S.events(), matches=S.matches();
  let plan=0,act=0;
  events.forEach(ev=>{
    const inLineup=ev.teams && Object.values(ev.teams).some(arr=>Array.isArray(arr)&&arr.some(n=>n&&alias.has(n)));
    if(!inLineup) return;
    plan++;
...
// 出席率明細：列出每一場「有被排進排表系統（含候補）」的場次，以及當天是否有實際出賽紀錄
...
function openAttendanceDetail(name){
  ...
  document.getElementById('att-detail-summary').innerHTML = r.plan
    ? `出席率：<strong style="color:${r.rate>=80?'var(--ok)':r.rate>=50?'var(--gold)':'var(--bad)'}">${r.rate}%</strong>　（實際出賽 ${r.act} 場 ／ 被排入排表 ${r.plan} 場，含候補）`
    : '此成員尚未被排入任何場次的排表，暫無出席率資料';
...
function renderAdminMembers(pane){
  ...
  const tbody=filtered.map(m=>{
    ...
    return `<tr>
      ...
      <td>${(function(){const r=_attRate(m.name);const c=r.rate===null?'var(--txt3)':r.rate>=80?'var(--ok)':r.rate>=50?'var(--gold)':'var(--bad)';return `<button class="btn btn-outline xs" style="color:${c};font-weight:700" onclick="openAttendanceDetail('${m.name.replace(/'/g,"\\'")}')">${r.act}/${r.plan}${r.rate===null?'':' ('+r.rate+'%)'}</button>`;})()}</td>
  ...
  pane.innerHTML=`<div class="sec-head">
    ...
    <div class="sec-actions">
      ...
      <button class="btn btn-outline sm" onclick="rebuildFromLineups()" title="若排表裡有名字但成員清單查無此人，點此補回">🔧 從排表補齊成員</button>
    </div>
  </div>
  ...
  <div class="tbl-wrap"><table>
    ...
    <tbody id="members-tbody">${tbody||'<tr><td colspan="5" style="text-align:center;color:var(--txt3);padding:24px">尚無成員</td></tr>'}</tbody>
  </table></div>`;
}
...
function filterMembers(){
  ...
  tb.innerHTML=f.map(x=>{
    ...
    return `<tr><td><strong>${x.name}</strong>${(x.changeLog&&x.changeLog.length)?`<br><span style="font-size:10px;color:var(--txt3)" title="${x.changeLog.map(c=>c.t+' '+c.type+'：'+c.from+'→'+c.to).join('&#10;')}">📝 ${x.changeLog[x.changeLog.length-1].t} ${x.changeLog[x.changeLog.length-1].type}</span>`:''}</td><td><span class="pill pill-job" style="background:${job.color};color:#fff">${job.name}</span></td><td>${(function(){const r=_attRate(x.name);const c=r.rate===null?'var(--txt3)':r.rate>=80?'var(--ok)':r.rate>=50?'var(--gold)':'var(--bad)';return `<button class="btn btn-outline xs" style="color:${c};font-weight:700" onclick="openAttendanceDetail('${x.name.replace(/'/g,"\\'")}')">${r.act}/${r.plan}${r.rate===null?'':' ('+r.rate+'%)'}</button>`;})()}</td><td style="font-size:11px;color:var(--txt2);max-width:130px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${(x.skills||[]).join('、')||'—'}</td><td><button class="btn btn-outline xs" onclick="openEditMember('${x.id}')">編輯</button></td></tr>`;
  }).join('')||'<tr><td colspan="5" style="text-align:center;color:var(--txt3);padding:24px">無符合</td></tr>';
}
...
function rebuildFromMatches(){
  ...
  S.matches().forEach(mt=>{
    (mt.players||[]).forEach(p=>{
      ...
      members.push({id:uid(),name:p.name,jobId:job?job.id:'',team:'候補',status:'一般成員',note:'',skills:[],baijia:[],createdAt:Date.now(),updatedAt:Date.now()});
