...
function openAttendanceDetail(name){
  ...
  {
    const noteLine = r.plan===0
      ? (r.totalPlayed>0 ? '（有出場紀錄，但當時未正式報名出席）' : '（尚無出場或報名紀錄）')
      : '';
    document.getElementById('att-detail-summary').innerHTML =
      `出席：<strong style="color:${attRateColor(r.totalPlayed,r.plan)}">${r.totalPlayed}/${r.plan}</strong>　（實際出場次數／已報名且有紀錄應出賽）${noteLine?'<br><span style="font-size:12px;color:var(--txt2)">'+noteLine+'</span>':''}`;
  }
  // 每場活動顯示三項狀態：報名／排表／出賽
  ...
  const tbody=filtered.map((m,idx)=>{
    ...
    return `<tr${inactive?' style="opacity:.55"':''}>
      ...
      <td>${(function(){
        ...
        return `<button class="btn btn-outline xs" style="color:${c};font-weight:700" onclick="openAttendanceDetail('${m.name.replace(/'/g,"\\'")}')">${a.totalPlayed}/${a.plan}</button><br><span style="font-size:10px;color:var(--txt3)">（實際出場次數／已報名且有紀錄應出賽）</span>`;
      })()}</td>
