...
//   b = 報名次數：累計「報名出席」的次數（每個場次算1次，不論當天打幾場比賽）
//   c = 排入排表次數：累計被排入「實際戰鬥小隊」（不含候補）的次數；
//       若當天已有比賽紀錄，依當天比賽場數計算（雙場周算2次）；還沒有比賽紀錄則算1次
//   → 候補（未被排入戰鬥小隊）不計入 c，也不會被視為缺席異常
function _attRate(name){
  ...
  events.forEach(ev=>{
    ...
    if(inCombatSquad(ev)){
      ...
      const day=matches.filter(mm=>String(mm.date||'').slice(0,10)===d);
      c += day.length>0 ? day.length : 1; // 雙場周（當天2場比賽）算2次；還沒有比賽紀錄先算1次
    }
  });
...
function openAttendanceDetail(name){
  ...
  {
    const note=attStatusNote(r.a,r.b,r.c,false);
    const noteColor=note?.type==='warn1'?'var(--gold)':note?.type==='warn2'?'var(--bad)':'var(--txt2)';
    const leaveNote=checkLongLeaveWarning(name);
    document.getElementById('att-detail-summary').innerHTML =
      `出席：<strong style="color:${attRateColor3(r.a,r.b,r.c)}">${r.a}/${r.b}/${r.c}</strong>　（實際出場次數／報名次數／排入排表次數）${note?'<br><span style="font-size:12px;color:'+noteColor+'">'+note.text+'</span>':''}${leaveNote?'<br><span style="font-size:12px;color:var(--bad)">'+leaveNote.text+'</span>':''}`;
  }
  // 每場活動顯示三項狀態：報名／排表／出賽
...
function _memberView(){
  ...
  members.forEach(m=>{
    ...
    actMap[m.id]={a:r.a, b:r.b, c:r.c, resp};
  });
...
function _memberRowsHtml(filtered, actMap, qa){
  const tbody=filtered.map((m,idx)=>{
    ...
    return `<tr${inactive?' style="opacity:.55"':''}>
      ...
      <td>${(function(){
        const c=attRateColor3(a.a,a.b,a.c);
        const note=attStatusNote(a.a,a.b,a.c,false);
        const leaveNote=checkLongLeaveWarning(m.name);
        const badge=note?` <span title="${note.text}" style="cursor:help">📢</span>`:'';
