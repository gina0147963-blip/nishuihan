...
function _attRate(name){
  ...
  events.forEach(ev=>{
    ...
    // 以「比賽紀錄」為單位：當天有幾場比賽，應出賽（分母）就算幾場；
    // 實際出賽（分子）依他真正出現在幾場比賽紀錄裡累加，跟「出場次數」的算法一致
    plan+=day.length;
    day.forEach(mm=>{ if((mm.participants||[]).some(n=>alias.has(n))) act++; });
  });
...
function openAttendanceDetail(name){
  ...
  {
    const note = attRateNote(r.totalPlayed, r.plan);
    const noteColor = note?.type==='good' ? 'var(--ok)' : note?.type==='warn' ? 'var(--gold)' : 'var(--txt2)';
    document.getElementById('att-detail-summary').innerHTML =
      `出席：<strong style="color:${attRateColor(r.totalPlayed,r.plan)}">${r.totalPlayed}/${r.plan}</strong>　（實際出場次數／已報名且有紀錄應出賽）${note?'<br><span style="font-size:12px;color:'+noteColor+'">'+note.admin+'</span>':''}`;
  }
  // 每場活動顯示三項狀態：報名／排表／出賽
  ...
  const rowCardHtml=x=>`
      <div style="border:1px solid var(--border);border-radius:8px;padding:8px 12px">
        ...
        <div style="display:flex;gap:14px;flex-wrap:wrap;font-size:13px">
          ...
          <span>出賽：${playBadge(x)}${x.dayMatchCount>1?' <span style="color:var(--txt3);font-weight:400">（當天共'+x.dayMatchCount+'場比賽'+(x.dayPlayedCount>0?'，計入出場次數'+x.dayPlayedCount+'次':'，但未出現在任何一場的紀錄中')+'）</span>':''}</span>
        </div>
      </div>`;
...
function _memberView(){
  ...
  members.forEach(m=>{
    ...
    actMap[m.id]={totalPlayed:r.totalPlayed, plan:r.plan, act:r.act, rate:r.rate, resp};
  });
  ...
  if(qa==='never'){
    filtered=filtered.filter(m=>{const a=actMap[m.id];return a.totalPlayed===0&&a.resp===0;});
  } else if(qa==='low'){
    filtered=filtered.slice().sort((a,b)=>(actMap[a.id].totalPlayed-actMap[b.id].totalPlayed)||(actMap[a.id].resp-actMap[b.id].resp));
  } else if(qa==='high'){
    filtered=filtered.slice().sort((a,b)=>(actMap[b.id].totalPlayed-actMap[a.id].totalPlayed)||(actMap[b.id].resp-actMap[a.id].resp));
  }
...
function _memberRowsHtml(filtered, actMap, qa){
  const tbody=filtered.map((m,idx)=>{
    ...
    const a=actMap[m.id];
    const inactive=a.totalPlayed===0&&a.resp===0;
    return `<tr${inactive?' style="opacity:.55"':''}>
      ...
      <td>${(function(){
        const c=attRateColor(a.totalPlayed,a.plan);
        const note=attRateNote(a.totalPlayed,a.plan);
        const badge=note?` <span title="${note.admin}" style="cursor:help">${note.type==='good'?'🙏':'📢'}</span>`:'';
        return `<button class="btn btn-outline xs" style="color:${c};font-weight:700" onclick="openAttendanceDetail('${m.name.replace(/'/g,"\\'")}')">${a.totalPlayed}/${a.plan}</button>${badge}<br><span style="font-size:10px;color:var(--txt3)">（實際出場次數／已報名且有紀錄應出賽）</span>`;
      })()}</td>
