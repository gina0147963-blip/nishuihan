...
function renderAdminMembers(pane){
  ...
  const wasTyping=document.activeElement&&document.activeElement.id==='ms-q'; // 必須在重繪前捕捉，重繪後焦點就消失了
  ...
  pane.innerHTML=`<div class="sec-head">
    <h2>👥 成員管理 <span style="font-size:12px;color:var(--txt2);font-weight:400">（${ORG_LABEL()}專屬，${members.length} 人${qa?'，顯示 '+filtered.length+' 人':''}）</span></h2>
    <div class="sec-actions">
      ...
      <button class="btn btn-outline sm" onclick="dedupeMembers()" title="找出名字完全相同的重複成員，自動合併成一筆（保留資料較完整的那筆，技能與曾用名會合併）">🧹 清理重複成員</button>
  ...
  <div class="filter-bar">
    ...
    <select id="ms-act" class="fsel" onchange="filterMembers()" title="依活動度排序或篩選，協助找出可能已離開幫派的成員">
      ...
      <option value="low">出席少 → 多</option>
      <option value="high">出席多 → 少</option>
      <option value="never">💤 只看從未出賽/報名</option>
    </select>
  </div>
  <div class="tbl-wrap"><table>
    <thead><tr><th>角色名稱</th><th>職業</th><th>出席場數</th><th>絕技</th><th>操作</th></tr></thead>
    <tbody id="members-tbody">${tbody||'<tr><td colspan="5" style="text-align:center;color:var(--txt3);padding:24px">'+(qa==='never'?'沒有「從未出賽/報名」的成員 🎉':'尚無成員')+'</td></tr>'}</tbody>
  </table></div>`;
  // 重新渲染後保留原本的篩選條件；若使用者正在搜尋框打字，還要恢復焦點與游標位置
  if(qs){const el=document.getElementById('ms-q');if(el)el.value=qs;}
  if(qc){const el=document.getElementById('ms-cls');if(el)el.value=qc;}
  if(qa){const el=document.getElementById('ms-act');if(el)el.value=qa;}
  if(wasTyping){const el=document.getElementById('ms-q');if(el){el.focus();el.setSelectionRange(el.value.length,el.value.length);}}
}
...
function filterMembers(){
  // 統一交給 renderAdminMembers 重繪（它會先讀取目前篩選條件、重繪後再回填），
  // 避免這裡再維護第二份表格產生邏輯，導致活動度篩選等新功能行為不一致
  renderAdminMembers(document.getElementById("pane-a-members"));
}
...
// ============================================================
// 清理重複成員：名字完全相同的合併成一筆
// 產生原因：多台裝置在同步完成前各自新增同一人（或改版遷移時本機與雲端各有一筆），
// 因為內部 id 不同，雲端合併時會被當成兩筆不同資料而都保留下來。
// 合併原則：保留「資料較完整」的那筆（有職業 > 技能較多 > 更新時間較新），
// 技能、群俠技能、曾用名取聯集；被移除的 id 會加入刪除墓碑，避免下次同步又從雲端復活。
// ============================================================
async function dedupeMembers(){
  const members=S.members();
  const byName={};
  members.forEach(m=>{ (byName[m.name]=byName[m.name]||[]).push(m); });
  const dupGroups=Object.values(byName).filter(g=>g.length>1);
  if(!dupGroups.length){ toast('沒有發現重複成員 🎉','ok'); return; }
  const total=dupGroups.reduce((s,g)=>s+g.length-1,0);
  if(!confirm('發現 '+dupGroups.length+' 個名字有重複（共可清除 '+total+' 筆多餘資料）。\n將自動保留資料較完整的一筆，技能與曾用名會合併。\n確定要清理嗎？')) return;

  const removedIds=[];
  const keepMap={};
  dupGroups.forEach(g=>{
    // 評分：有職業+2、每個技能+1、有群俠+1，同分再比更新時間新的優先
    const score=m=>((m.jobId&&m.jobId!=='unknown')?2:0)+((m.skills||[]).length)+((m.baijia||[]).length?1:0);
    const sorted=g.slice().sort((a,b)=>(score(b)-score(a))||((b.updatedAt||0)-(a.updatedAt||0)));
    const keep={...sorted[0]};
    // 合併其餘筆的技能/群俠/曾用名（取聯集）
    sorted.slice(1).forEach(m=>{
      keep.skills=[...new Set([...(keep.skills||[]),...(m.skills||[])])];
      keep.baijia=[...new Set([...(keep.baijia||[]),...(m.baijia||[])])];
      keep.aliases=[...new Set([...(keep.aliases||[]),...(m.aliases||[])])];
      if(!keep.note&&m.note) keep.note=m.note;
      if(keep.status!=='固定團'&&m.status==='固定團') keep.status='固定團';
      removedIds.push(m.id);
    });
    keep.updatedAt=Date.now();
    keepMap[keep.id]=keep;
  });

  const result=[];
  const removedSet=new Set(removedIds);
  members.forEach(m=>{
    if(removedSet.has(m.id)) return;          // 被合併掉的不保留
    result.push(keepMap[m.id]||m);            // 保留筆用合併後的版本
  });
  // 加入刪除墓碑：避免同步時雲端還留著的舊筆又被合併回來
  if(typeof _addTomb==='function') removedIds.forEach(id=>_addTomb('members', id));
  S.setMembers(result);
  toast('✅ 已清理 '+removedIds.length+' 筆重複成員（合併保留 '+dupGroups.length+' 筆）','ok');
  renderAdminMembers(document.getElementById('pane-a-members'));
}
