...
async function syncLoad(silent) {
  ...
  try {
    ...
    _suppressWrite = true;
    try {
      if (Array.isArray(data.members))                              _writeMembers(data.members);
      ...
      if (Array.isArray(data.events))                               _writeEvents(data.events);
      if (data.signups)                                             _writeSignups(data.signups);
      if (Array.isArray(data.matches))                              _writeMatches(data.matches);
    } finally {
...
async function testConnection(){
  ...
  try{
    ...
    toast(data.ok?('✅ 連線成功：'+(data.message||'')):('⚠️ 回應異常：'+JSON.stringify(data).slice(0,80)),data.ok?'ok':'err');
  }catch(err){
