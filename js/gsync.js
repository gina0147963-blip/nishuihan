...
async function syncLoad(silent) {
  ...
  try {
    ...
    const res = await fetch(_syncUrl, {
      ...
      body: JSON.stringify({ action: 'load', mode: reqMode }),
    });
    ...
    if (CUR_MODE !== reqMode) {
      console.warn('syncLoad: 模式已於同步期間變更（'+reqMode+' → '+CUR_MODE+'），放棄套用此次結果，避免資料寫錯模式');
      return;
    }
...
function _buildSyncPayload(mode){
  return {
    action:    'sync',
    mode:      mode,
...
async function _doWrite() {
  ...
  const payload = _buildSyncPayload(CUR_MODE); // 在任何 await 之前，同一瞬間打包好模式標籤+資料內容
  try {
...
async function syncWriteNowFast(){
  ...
  try{
    if (CUR_MODE !== reqMode){ _pendingWrite=false; return false; }
    const payload = _buildSyncPayload(reqMode);
    await _pushToCloud(payload);
    _pendingWrite = false;
    const now = _nowStr();
    S.setConfig({ ...S.config(), lastSync: now });
...
async function syncWriteNow(maxRetries){
  ...
  const reqMode = CUR_MODE; // 從頭到尾都用這個模式，不管中途 CUR_MODE 有沒有被切換
  clearTimeout(_autoSaveTimer);
  clearTimeout(_writeRetryTimer);
  _pendingWrite = true;
  for (let i=0; i<maxRetries; i++){
    ...
    try{
      ...
      try{
        ...
        if (loadRes.ok && CUR_MODE===reqMode){
          const cloud = await loadRes.json();
          if (cloud && !cloud.error && CUR_MODE===reqMode){
      ...
      if (CUR_MODE !== reqMode){
        console.warn('syncWriteNow: 模式已於同步期間變更（'+reqMode+' → '+CUR_MODE+'），放棄這次寫入');
        _pendingWrite = false;
        return false;
      }
      const payload = _buildSyncPayload(reqMode); // 在送出前的最後一刻，同一瞬間打包模式標籤+資料內容
      await _pushToCloud(payload);
      _pendingWrite = false;
      const now = _nowStr();
      S.setConfig({ ...S.config(), lastSync: now });
...
window.addEventListener('pagehide', () => {
  if (_pendingWrite && _syncUrl && navigator.sendBeacon) {
    try {
      const payload = JSON.stringify(_buildSyncPayload(CUR_MODE));
