...
// 建立「舊名字 → 本人(改名後)的名字」對照表，供報名/排表的改名遷移共用。
// ★ 關鍵保護：如果某個「曾用名」剛好也是「另一位目前現役成員」正在使用的本名
// （同名但不同人——例如舊名字後來被別人拿去用、或單純兩個相似名字的不同玩家），
// 就排除、不列入這張對照表。這種情況系統無法安全判斷一筆紀錄到底屬於誰，
// 寧可不搬（維持現狀，留給管理員自己確認），也不能誤把「現役同名成員」自己
// 才剛排進去的排表格子／報名紀錄，當成別人的舊名字搬走、造成該現役成員憑空消失。
function _buildAliasOwnerMap(members){
  const currentNames=new Set(members.map(m=>m.name));
  const aliasOwner=new Map();
  members.forEach(m=>{
    (m.aliases||[]).forEach(a=>{
      if(a===m.name) return;
      if(currentNames.has(a)) return; // 撞名到另一位現役成員的本名，跳過不搬
      aliasOwner.set(a, m.name);
    });
  });
  return aliasOwner;
}
...
function _migrateAliasSignups(){
  ...
  const aliasOwner=_buildAliasOwnerMap(members);
...
function _migrateAliasLineups(){
  const members=S.members();
  const aliasOwner=_buildAliasOwnerMap(members); // 舊名字 → 本人(改名後)的名字（已排除撞名到現役成員本名的情況）
  if(!aliasOwner.size) return false;
  ...
  events.forEach(ev=>{
    ...
    // ★ 去重必須「整場活動」一起看，不能只看單一小隊：如果改名後的新名字剛好已經
    // 出現在「另一個」小隊裡（例如改名前後兩次被拖進不同小隊），只在各自小隊內去重
    // 會漏掉跨小隊的重複，導致同一人同時出現在兩個小隊。用同一個 seen 集合，
    // 依小隊順序（候補最後處理）掃過整場活動，先出現的保留，後面重複的淨空/移除。
    if(ev.teams){
      const seen=new Set();
      const squadKeys=Object.keys(ev.teams).filter(k=>k!=='reserve'&&Array.isArray(ev.teams[k]));
      ...
      if(Array.isArray(ev.teams.reserve)){
        const arr=ev.teams.reserve;
        let localChanged=false;
        const out=[];
        arr.forEach(n=>{
          if(!n) return;
          ...
          else localChanged=true; // 已經在戰鬥小隊或更前面的候補中出現過，這筆重複的候補移除
        });
        if(localChanged){ ev.teams.reserve=out; changed=true; }
      }
    }
