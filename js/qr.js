...
// 同時網址也帶上目前設定的 Google Apps Script 同步網址（?gs=...），
// 讓全新裝置（從沒設定過同步）掃碼後也能自動同步到成員名單，
// 才能在登入頁用關鍵字搜尋到自己的角色名稱（見 app.js 的 loginSuggest / DOMContentLoaded）。
// ============================================================

function _qrShareUrl(){
  let url = location.origin + location.pathname + '?mode=' + CUR_MODE;
  const cfg = S.config();
  if (cfg && cfg.gsWebhook) url += '&gs=' + encodeURIComponent(cfg.gsWebhook);
  return url;
}
