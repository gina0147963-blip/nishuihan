...
// 雲端同步網址已固定寫死在 gsync.js（GS_URL），全新裝置掃碼後會自動同步成員名單。
// ============================================================

function _qrShareUrl(){
  // 同步網址已固定寫死在 gsync.js 的 GS_URL，連結只需帶模式與組織代號，
  // 掃碼的新裝置會自動連上雲端並直接進入該組織的登入頁
  return location.origin + location.pathname + '?mode=' + CUR_MODE + '&org=' + encodeURIComponent(CUR_ORG);
}

function openQRModal(){
  const url = _qrShareUrl();
  const label = ORG_LABEL() || CUR_MODE;
...
function downloadQRImage(){
  ...
  a.href = dataUrl;
  a.download = '報名QR碼_' + (ORG_LABEL() || CUR_MODE) + '.png';
  a.click();
  toast('QR圖片已下載', 'ok');
}
