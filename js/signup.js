...
function renderAdminSignupMgr(pane){
  ...
  ${events.length?events.map(ev=>{
    ...
    // 暫離成員不需回覆出席/請假/候補，不列入未回覆與催繳名單
    const pendingNames=members.filter(m=>m.status!=='暫離'&&!responded.has(m.name)).map(m=>m.name);
...
async function adminSetSignup(evId,name,status){
  if(status==='reserve'){
  ...
  // 立即送出同步，不等待背景的2秒防抖排程——避免管理員改完馬上重整頁面，
  // 導致排程還沒觸發就被中斷，資料只留在本機沒真正送上雲端
  const ok=await syncWriteNowFast();
  if(ok===true) toast('✅ 已將「'+name+'」調整為「'+label+'」，並已同步','ok');
  else if(ok==='nosync') toast('⚠️ 這台裝置尚未連接雲端同步','err');
  else toast('⚠️ 「'+name+'」已存在本機，但雲端同步失敗，背景將自動重試（請勿太快關閉頁面）','err');
}
...
function copyPendingReminder(evId){
  ...
  const responded=new Set(Object.keys(evS));
  const pendingNames=S.members().filter(m=>m.status!=='暫離'&&!responded.has(m.name)).map(m=>m.name);
