  ...
  ${matches.length?matches.map(m=>{
    ...
    return `<div class="match-card">
      ...
      <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;justify-content:center">
        ...
        <button class="btn btn-outline sm" onclick="openMatchEdit('${m.id}')" title="修正活動日期或勝敗結果（CSV匯入偶爾會有誤植）">✏️ 編輯</button>
...
function _ensureMatchEditModal(){
  ...
  div.innerHTML=`<div class="modal">
    ...
    <div class="modal-ft">
      <button class="btn btn-outline" onclick="closeModal('modal-match-edit')">取消</button>
      <button class="btn btn-blue" onclick="saveMatchEdit()">儲存</button>
    </div>
  </div>`;
  document.body.appendChild(div);
}
function openMatchEdit(id){
  const m=S.matches().find(x=>x.id===id); if(!m){ toast('找不到此比賽紀錄','err'); return; }
