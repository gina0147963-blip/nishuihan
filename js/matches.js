  ...
  ${matches.length?matches.map(m=>{
    ...
    return `<div class="match-card">
      ...
      <div class="match-info">
        ...
        ${(m.videos&&m.videos.length)?`<div style="margin-top:6px;display:flex;gap:10px;flex-wrap:wrap">${m.videos.map((u,i)=>{
          const label=/youtu\.?be/i.test(u)?'YouTube':/bilibili/i.test(u)?'Bilibili':/twitch/i.test(u)?'Twitch':'影片';
          return `<a href="${u}" target="_blank" rel="noopener" style="font-size:13px;font-weight:700;color:var(--accent);text-decoration:underline">🎬 ${label}${m.videos.length>1?' '+(i+1):''}</a>`;
        }).join('')}</div>`:''}
      </div>
...
function _ensureVideoModal(){
  ...
  div.innerHTML=`<div class="modal">
    <div class="modal-hd"><h3>🎬 比賽影片備註</h3><button class="close-btn" onclick="closeModal('modal-match-videos')">✕</button></div>
    <div class="modal-body">
      <div class="fg"><label>貼上影片網址（一行一個，可留空，之後隨時回來補充）</label>
        <textarea id="match-videos-input" class="fi" rows="5" placeholder="https://youtube.com/...&#10;https://www.bilibili.com/..." style="resize:vertical;min-height:100px"></textarea>
      </div>
      <p class="hint">儲存後會顯示在該場比賽卡片上，點一下就能開啟影片。沒有 https:// 開頭會自動幫你補上。</p>
    </div>
    <div class="modal-ft">
...
function openMatchVideos(id){
  ...
  _ensureVideoModal();
  _videoEditId=id;
  const ta=document.getElementById('match-videos-input');
  if(ta) ta.value=(m.videos||[]).join('\n');
...
async function saveMatchVideos(){
  ...
  const ta=document.getElementById('match-videos-input');
  // 一行一個網址；沒有 http(s):// 開頭的自動補上 https://（貼 www.youtube.com/... 也能用）
  const urls=(ta?ta.value:'').split('\n').map(s=>s.trim()).filter(Boolean)
    .map(u=>/^https?:\/\//i.test(u)?u:'https://'+u);
  const matches=S.matches();
  const idx=matches.findIndex(x=>x.id===_videoEditId);
  matches[idx]={...m, videos:urls, updatedAt:Date.now()};
  S.setMatches(matches);
  closeModal('modal-match-videos');
  toast(urls.length?('✅ 已儲存 '+urls.length+' 個影片網址'):'已清空影片網址','ok');
  renderAdminMatches(document.getElementById('pane-a-matches'));
}
