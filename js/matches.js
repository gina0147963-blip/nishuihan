  ...
  ${matches.length?matches.map(m=>{
    ...
    return `<div class="match-card">
      ...
      <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;justify-content:center">
        ...
        <button class="btn btn-outline sm" onclick="swapMatchCamps('${m.id}')" title="匯入時我方/對方判定錯誤時使用：對調雙方陣營，並協助清除被誤加進成員管理的對方玩家">🔁 我方對調</button>
...
// ═══ 我方/對方陣營對調（修復匯入時判定錯誤的比賽）═══
// 1) 對調該場比賽所有玩家的陣營標籤、雙方人數、對手名稱、participants
// 2) 掃描「被誤加進成員管理的對方玩家」：名字在對調後的對方名單中、
//    且沒有任何報名紀錄與排表紀錄（代表只是被誤加、從未實際參與）→ 詢問後批次刪除（含刪除記號防復活）
async function swapMatchCamps(matchId){
  const matches=S.matches();
  const mt=matches.find(x=>x.id===matchId);
  if(!mt){ toast('找不到此紀錄','err'); return; }
  if(!Array.isArray(mt.players)||!mt.players.length){ toast('此紀錄沒有玩家數據，無法對調','err'); return; }
  const ourNew=mt.players.filter(p=>p.camp==='對方').map(p=>p.name);
  const enemyNew=mt.players.filter(p=>p.camp==='我方').map(p=>p.name);
  if(!confirm('確定對調此場比賽的我方/對方？\n\n對調後我方 '+ourNew.length+' 人、對方 '+enemyNew.length+' 人\n（適用於匯入時判定錯誤的情況）')) return;

  // 找出目前的雙方俱樂部名稱：notes 檔名裡通常有，退而求其次用互換 enemy 欄位
  mt.players.forEach(p=>{ p.camp=(p.camp==='我方')?'對方':'我方'; });
  const oldEnemy=mt.enemy||'';
  // 嘗試從匯入檔名（notes）找出我方原始名稱來當新的對手名
  let newEnemy='';
  const nm=String(mt.notes||'').match(/_([^_]+)_([^_.]+)\.csv/);
  if(nm){ newEnemy=(nm[1]===oldEnemy)?nm[2]:nm[1]; }
  mt.enemy=newEnemy||('原我方（'+oldEnemy+' 對調）');
  const oc=mt.ourCount; mt.ourCount=mt.enemyCount; mt.enemyCount=oc;
  mt.participants=mt.players.filter(p=>p.camp==='我方').map(p=>p.name);
  ...
  // 清理被誤加的對方玩家
  const enemySet=new Set(enemyNew);
  const signups=S.signups();
  const hasSignup=name=>Object.values(signups).some(evS=>evS&&evS[name]!==undefined);
  const inLineup=name=>S.events().some(ev=>ev.teams&&Object.values(ev.teams).some(arr=>Array.isArray(arr)&&arr.includes(name)));
  const inOtherMatch=name=>S.matches().some(m2=>m2.id!==matchId&&(m2.participants||[]).includes(name));
  const suspects=S.members().filter(m=>enemySet.has(m.name)&&!hasSignup(m.name)&&!inLineup(m.name)&&!inOtherMatch(m.name));
  if(suspects.length){
    if(confirm('偵測到 '+suspects.length+' 位「被誤加的對方玩家」（在對方名單中、且無任何報名/排表/其他出賽紀錄）：\n'+suspects.slice(0,15).map(m=>m.name).join('、')+(suspects.length>15?'…等':'')+'\n\n要一併從成員管理刪除嗎？（會登記刪除記號，不會復活）')){
      const removeIds=new Set(suspects.map(m=>m.id));
      S.setMembers(S.members().filter(m=>!removeIds.has(m.id)));
      if(typeof _addTomb==='function') removeIds.forEach(id=>_addTomb('members', id));
      toast('✅ 已刪除 '+removeIds.size+' 位誤加的對方玩家','ok');
      if(typeof syncWriteNow==='function') await syncWriteNow();
    }
  } else {
    toast('沒有偵測到需要清理的誤加成員','');
  }
}