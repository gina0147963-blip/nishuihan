...
function renderAdminMatches(pane){
  ...
  ${matches.length?matches.map(m=>{
    ...
    return `<div class="match-card">
      ...
      <div style="display:flex;gap:4px;flex-direction:column"><button class="btn btn-blue xs" onclick="openMatchDetail('${m.id}')">📊 數據</button><button class="btn btn-red xs" onclick="deleteMatchById('${m.id}')">刪除</button></div>
    </div>`;
...
function openMatchDetail(id){
  ...
  const cols=[['kills','擊敗'],['assist','助攻'],['resource','資源'],['pDamage','對玩家傷害'],['bDamage','對建築傷害'],['heal','治療值'],['taken','承受傷害'],['hurt','重傷'],['revive','化羽/清泉'],['burn','焚骨']];
  const section=(camp,label)=>{
    const rows=(m.players||[]).filter(pl=>pl.camp===camp).sort((a,b)=>(b.pDamage||0)-(a.pDamage||0));
    if(!rows.length) return '';
    return `<h4 style="margin:10px 0 6px">${label}（${rows.length}人）</h4>
    <div class="tbl-wrap"><table style="font-size:11px">
      <thead><tr><th>玩家</th><th>職業</th>${cols.map(c=>`<th>${c[1]}</th>`).join('')}</tr></thead>
      <tbody>${rows.map(pl=>`<tr><td style="white-space:nowrap"><strong>${pl.name}</strong></td><td>${pl.job||''}</td>${cols.map(c=>`<td>${fmtNum(pl[c[0]]||0)}</td>`).join('')}</tr>`).join('')}</tbody>
    </table></div>`;
  };
  document.getElementById('match-detail-body').innerHTML=`
    <div style="margin-bottom:8px;font-size:13px"><b>${m.date||''}</b> ／ vs ${m.enemy||''} ／ <span style="color:${m.result==='勝利'?'var(--ok)':'var(--bad)'}">${m.result||''}</span></div>
    ${section('我方','🔵 我方')}${section('對方','🔴 對方')}
    ${!(m.players||[]).length?'<p class="hint">此紀錄無詳細數據（可能為舊資料）</p>':''}`;
