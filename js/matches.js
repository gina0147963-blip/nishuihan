  ...
  ${matches.length?matches.map(m=>{
    ...
    return `<div class="match-card">
      ...
      <div class="match-info">
        ...
        ${(m.videos&&m.videos.length)?`<div style="margin-top:4px;display:flex;gap:8px;flex-wrap:wrap">${m.videos.map((u,i)=>`<a href="${u}" target="_blank" rel="noopener" style="font-size:12px;color:var(--accent)">🎬 影片${m.videos.length>1?(i+1):''}</a>`).join('')}</div>`:''}
      </div>
...
async function saveMatchVideos(){
  ...
  // 簡單防呆：不是網址格式的行給予提醒但仍允許儲存（可能是內部代號）
  const bad=urls.filter(u=>!/^https?:\/\//i.test(u));
  if(bad.length && !confirm('以下內容看起來不是網址（建議以 http:// 或 https:// 開頭）：\n'+bad.join('\n')+'\n\n仍要儲存嗎？')) return;
