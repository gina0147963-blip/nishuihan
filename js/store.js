...
const S = {
  ...
  // setSignups 會自動比對差異，把「這台裝置真正改過」的報名鍵記到 dirty 清單（48小時後過期）。
  // 雲端同步合併時只有 dirty 的本機值才能覆蓋雲端，拿著舊資料的裝置不會再把別人較新的報名蓋掉。
  setSignups(v){
    try{
      const prev=this.signups()||{};
      const dirtyKey=this.k('signup_dirty');
      const dirty=this.g(dirtyKey,{})||{};
      const now=Date.now();
      const evIds=new Set([...Object.keys(prev),...Object.keys(v||{})]);
      evIds.forEach(evId=>{
        const a=prev[evId]||{}, b=(v||{})[evId]||{};
        const names=new Set([...Object.keys(a),...Object.keys(b)]);
        names.forEach(n=>{ if(a[n]!==b[n]) dirty[evId+'|'+n]=now; });
      });
      Object.keys(dirty).forEach(k=>{ if(now-dirty[k]>172800000) delete dirty[k]; }); // 48h 過期
      this.s(dirtyKey, dirty);
    }catch(_){}
    return this.s(this.k('signups'),v);
  },
  signupDirty(){ return this.g(this.k('signup_dirty'),{})||{}; },
