...
function renderPlayerHome(pane){
  ...
  if(latestEvent){
    ...
    const extraInfo=latestEvent.type==='約戰'
      ? `<div style="font-size:12px;color:var(--txt2);margin-bottom:10px">${latestEvent.eventTime?'🕐 活動時間：'+latestEvent.eventTime+'　':''}${latestEvent.matchFormat?'⚔️ 賽制 '+latestEvent.matchFormat:''}</div>`
      : '';
...
function renderPlayerSignup(pane){
  ...
  pane.innerHTML=`<div class="sec-head"><h2>✋ 活動報名</h2></div>`+events.map(ev=>{
    ...
    return `<div class="signup-card">
      ...
      <div class="signup-meta">📅 ${ev.date||''} ／ ${ev.type||''}${ev.type==='約戰'?(ev.eventTime?'　🕐 '+ev.eventTime:'')+(ev.matchFormat?'　⚔️ 賽制 '+ev.matchFormat:''):''}</div>
