  ...
  if(latestEvent){
    ...
    nextEvent=`<div class="card">
      ...
      <div style="display:flex;gap:8px">
        ...
        <button class="sbtn sbtn-mb ${myStatus==='reserve'?'sel':''}" ${(locked||_isFixedMe())?'disabled style="opacity:.4;cursor:not-allowed"':''} onclick="quickSignup('${latestEvent.id}','reserve')" title="${_isFixedMe()?'固定團成員為自動出席，不可候補':'可出席，但先以候補為主'}">🟡 候補</button>
