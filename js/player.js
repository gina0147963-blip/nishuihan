...
function renderPlayerHome(pane){
  ...
  if(latestEvent){
    ...
    // 約戰額外顯示活動時間與賽制
    const extraInfo=latestEvent.type==='約戰'
      ? `<div style="font-size:12px;color:var(--txt2);margin-bottom:10px">${latestEvent.eventTime?'🕐 活動時間：'+latestEvent.eventTime+'　':''}${latestEvent.matchFormat?'⚔️ '+(latestEvent.matchFormat==='2'?'兩局制':'一局制'):''}</div>`
      : '';
    nextEvent=`<div class="card">
      ...
      <div style="font-size:12px;color:var(--txt2);margin-bottom:${extraInfo?'6px':'10px'}">📅 ${latestEvent.date||''} ／ 目前 ${cnt} 人出席</div>
      ${extraInfo}
...
function renderPlayerSignup(pane){
  ...
  pane.innerHTML=`<div class="sec-head"><h2>✋ 活動報名</h2></div>`+events.map(ev=>{
    ...
    return `<div class="signup-card">
      ...
      <div class="signup-meta">📅 ${ev.date||''} ／ ${ev.type||''}${ev.type==='約戰'?(ev.eventTime?'　🕐 '+ev.eventTime:'')+(ev.matchFormat?'　⚔️ '+(ev.matchFormat==='2'?'兩局制':'一局制'):''):''}</div>
