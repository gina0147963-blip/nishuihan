...
function calcMyAttendance(){
  // 出席狀態算法與管理端「成員管理」的 _attRate 完全一致（兩邊務必同步，避免各算各的）：
  //   a = 實際出場次數／b = 報名次數（含候補）／c = 排入排表次數（含候補）
  //   bFull/cFull = 只算「出席」與「戰鬥小隊」、不含候補的部分，用來判斷 b、c 超出 a
  //   是候補沒被叫上場（正常）還是真的沒出席/沒出賽（異常）
  //   b、c 都依當天比賽場數計算（雙場周算2次），且都只計算「已有比賽紀錄」的場次
  ...
  function inCombatSquad(ev){
    ...
    for(const [key,arr] of Object.entries(ev.teams)){
      ...
      if(arr.some(n=>myNames.has(n))) return true;
    }
    return false;
  }
  function inReserveSquad(ev){
    return !!(ev.teams && (ev.teams.reserve||[]).some(n=>myNames.has(n)));
  }
  ...
  events.forEach(ev=>{
    ...
    let st=''; myNames.forEach(a=>{ if(evS[a]!==undefined) st=evS[a]; });
    if(st==='attend') bFull += day.length;
    else if(st==='reserve') bReserve += day.length;

    if(inCombatSquad(ev)) cFull += day.length;
    else if(inReserveSquad(ev)) cReserve += day.length;
  });
  ...
  matches.forEach(mm=>{
    ...
    if((mm.participants||[]).some(n=>myNames.has(n))) a++;
  });
...
function renderPlayerHome(pane){
  ...
  if(upcoming.length){
    nextEvent=upcoming.map((ev,i)=>{
      ...
      const locked=_isLocked(ev.id) || _isSuspendedMe();
      ...
      ${_isSuspendedMe()?'<p class="hint" style="color:var(--bad);margin-top:6px">⛔ 你目前為「暫離」狀態，需要管理員解除後才能報名</p>':locked?'<p class="hint" style="color:var(--bad);margin-top:6px">🔒 已截止報名</p>':''}
    </div>`;
    }).join('');
  } else {
...
function _buildSignupCardBody(ev, isPast){
  ...
  return `<div class="signup-opts">
      ...
      </div>`; // 影片上傳已改在「已結束場次」的摺疊列上直接顯示（見 renderPlayerSignup），這裡不再重複
}
// 出席/候補/請假/待回覆名單的展開收合
function toggleSignupLists(id){
  const box=document.getElementById(id);
  const arrow=document.getElementById(id+'-arrow');
  if(!box) return;
  const opening=box.classList.contains('hidden');
  box.classList.toggle('hidden');
  if(arrow) arrow.textContent=opening?'▴ 收合名單':'▾ 查看名單';
}
...
function _buildPlayerVideoSection(ev){
  ...
  const suspended=_isSuspendedMe();
  return `<div style="padding:0 14px 12px;border-bottom:1px solid var(--border)${suspended?';opacity:.5':''}">
      <div style="font-size:12px;color:var(--txt2);font-weight:700;margin-bottom:6px">🎬 上傳我的影片視角${myTeam?'（'+myTeam+'）':''}</div>
      <div style="display:flex;gap:6px;flex-wrap:wrap">
        ${dayMatches.length>1?`<select id="pv-match-${ev.id}" class="fsel sm" ${suspended?'disabled':''} style="flex:1 1 100px">${matchOpts}</select>`:`<input type="hidden" id="pv-match-${ev.id}" value="${dayMatches[0].id}">`}
        <input id="pv-url-${ev.id}" class="fi sm" ${suspended?'disabled':''} placeholder="貼上影片網址" style="flex:2 1 160px;min-width:0">
        <button class="btn btn-blue sm" ${suspended?'disabled style="cursor:not-allowed"':''} onclick="submitMyVideo('${ev.id}')" title="${suspended?'暫離狀態需管理員解除後才能上傳':''}">上傳</button>
      </div>
      ${suspended?'<p class="hint" style="color:var(--bad);margin-top:4px">⛔ 暫離狀態，需管理員解除後才能上傳</p>':''}
      ${mineHtml}
    </div>`;
}
...
function renderPlayerSignup(pane){
  ...
  const pastHtml=past.length?`
    <div style="margin-top:16px">
      <h3 style="font-size:14px;color:var(--txt2);margin-bottom:8px">🗂️ 已結束的場次（點日期展開查看報名狀態／名單）</h3>
