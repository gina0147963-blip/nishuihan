  ...
  if(latestEvent){
    ...
    nextEvent=`<div class="card">
      ...
      <div style="display:flex;gap:8px">
        ...
        <button class="sbtn ${myStatus==='reserve'?'sel':''}" style="border-color:var(--gold);color:var(--gold)${locked?';opacity:.4;cursor:not-allowed':''}" ${locked?'disabled':''} onclick="quickSignup('${latestEvent.id}','reserve')" title="可出席，但先以候補為主">🟡 候補</button>
  ...
  pane.innerHTML=`<div class="sec-head"><h2>✋ 活動報名</h2></div>`+events.map(ev=>{
    ...
    return `<div class="signup-card">
      ...
      <div class="signup-opts">
        ...
        <button class="sbtn ${my==='reserve'?'sel':''}" style="border-color:var(--gold);color:var(--gold)${locked?';opacity:.4;cursor:not-allowed':''}" ${locked?'disabled':''} onclick="doSignup('${ev.id}','reserve')" title="可出席，但先以候補為主">🟡 候補</button>
