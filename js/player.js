...
function renderPlayerSignup(pane){
  ...
  pane.innerHTML=`<div class="sec-head"><h2>✋ 活動報名</h2></div>`+events.map(ev=>{
    ...
    return `<div class="signup-card">
      ...
      ${locked?'<p class="hint" style="color:var(--bad)">🔒 已截止報名（活動前2天起鎖定，無法再更改）</p>':''}
...
function _isLocked(evId){
  ...
  // 活動前2天（00:00 起）到活動結束後皆鎖定，不可再更改報名
  const lockTime=new Date(evDay.getTime()-2*24*60*60*1000);
  return Date.now()>=lockTime.getTime();
}
async function doSignup(evId,status){
  if(_isLocked(evId)){ toast('此活動已於前2天截止，無法再更改報名','err'); return; }
  const label={attend:'出席',absent:'請假'}[status]||status;
  if(!confirm('確認回報「'+label+'」？\n（活動前2天截止前都可再修改）')) return;
  const s=S.signups();
  if(!s[evId])s[evId]={};
...
async function saveSelfEdit(){
  ...
  if(idx>=0){
    ...
    members.push({id:uid(),name:newName,jobId,status:'一般成員',skills,baijia,note:'',createdAt:Date.now(),updatedAt:Date.now()});
  }
  S.setMembers(members);
