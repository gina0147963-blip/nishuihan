  ...
  if(latestEvent){
    ...
    nextEvent=`<div class="card">
      ...
      <div style="display:flex;gap:8px">
        ...
        <button class="sbtn sbtn-mb ${myStatus==='reserve'?'sel':''}" ${locked?'disabled style="opacity:.4;cursor:not-allowed"':''} onclick="quickSignup('${latestEvent.id}','reserve')" title="可出席，但先以候補為主">🟡 候補</button>
  ...
  pane.innerHTML=`<div class="sec-head"><h2>✋ 活動報名</h2></div>`+events.map(ev=>{
    ...
    return `<div class="signup-card">
      ...
      <div class="signup-opts">
        ...
        <button class="sbtn sbtn-mb ${my==='reserve'?'sel':''}" ${locked?'disabled style="opacity:.4;cursor:not-allowed"':''} onclick="doSignup('${ev.id}','reserve')" title="可出席，但先以候補為主">🟡 候補</button>
