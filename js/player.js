...
function calcMyAttendance(){
  ...
  // 分子＝被排進排表的場次中，當天比賽紀錄也有出現；分母＝被排進排表系統（含候補）的場次數
  ...
  events.forEach(ev=>{
    const inLineup=ev.teams && Object.values(ev.teams).some(arr=>Array.isArray(arr)&&arr.some(n=>n&&myNames.has(n)));
    if(!inLineup) return;
    signedUp++;
...
async function togglePlayerFixedStatus(){
  ...
  toast('儲存中，正在同步到雲端...','ok');
  const ok=await syncWriteNow();
  if(ok) toast('✅ 已切換為「'+next+'」，並已同步','ok');
  else toast('⚠️ 已切換為「'+next+'」，但雲端同步失敗，請檢查網路後再確認一次','err');
...
async function quickSignup(evId, status){
  ...
  toast('儲存中，正在同步到雲端...','ok');
  const ok=await syncWriteNow();
  toast((prev===status?'已取消':labels[status])+(ok?'（已同步）':'（⚠️雲端同步失敗，請稍後再確認一次）'), ok?'ok':'err');
...
async function doSignup(evId,status){
  ...
  toast(label2+(ok?'（已同步）':'（⚠️雲端同步失敗，請稍後再確認一次）'), ok?'ok':'err');
...
async function saveSelfEdit(){
  ...
  const idx=members.findIndex(m=>m.name===CUR_USER);
  if(idx>=0){ members[idx]={...members[idx],jobId,skills,baijia,updatedAt:Date.now()}; }
  else { members.push({id:uid(),name:CUR_USER,jobId,status:'一般成員',skills,baijia,note:'',createdAt:Date.now(),updatedAt:Date.now()}); }
  S.setMembers(members);
  toast('儲存中，正在同步到雲端...','ok');
  const ok=await syncWriteNow();
  closeModal('modal-self-edit');
  if(ok) toast('✅ 資料已更新，並已同步到雲端','ok');
  else toast('⚠️ 資料已存在本機，但雲端同步失敗，請檢查網路後再打開一次確認','err');
