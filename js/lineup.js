  ...
  pane.innerHTML=`
    <div class="sec-head">
      ...
      <div class="sec-actions">
        ...
        <button class="btn btn-blue sm" onclick="openEventModal()">+ 新增場次</button>
...
function buildPoolHTML(cands, classFilter, searchFilter){
  // Accept filter values as parameters instead of reading from DOM
  // (DOM elements don't exist yet when this is called during initial render)
  const classF = classFilter||'';
  const searchF = (searchFilter||'').toLowerCase();
  let f=cands;
...
function createEvent(){
  try{
    ...
    const date=dateEl?dateEl.value:fmtDate(new Date());
    ...
    ensureTeams(newEv);
    events.push(newEv);
    S.setEvents(events);  // 自動觸發 gsync 寫入，不檢查回傳值（攔截後回傳可能不一致）
    ...
    toast('❌ 建立場次時發生錯誤：'+err.message,'err');
  }
}
