...
function exportLineupImage(){
  ...
  function drawTeam(ctx,ti,y){
    ...
    for(let i=0;i<SQUAD_COUNT;i++){
      ...
      const x=PAD+i*COLW;
      ctx.fillStyle=isGuard?'#3d1f12':'#1e2538';
      ctx.fillRect(x,y,COLW-4,SQHD-4);
      ctx.fillStyle=isGuard?'#ff5722':'#aeb6d0';
    ...
    for(let i=0;i<SQUAD_COUNT;i++){
      ...
      const x=PAD+i*COLW;
      ctx.strokeStyle='#ff5722'; ctx.lineWidth=4; // 橘紅色粗框：與所有職業代表色都不同，對比明顯
...
function _buildPlayerBoardHTML(ev){
  ...
  return `${buildSummaryBar(ev,members)}<div class="lineup-board">${teamsHTML||'<div class="empty"><div class="empty-ico">🗡️</div><p>尚未排表</p></div>'}</div>`;
}
