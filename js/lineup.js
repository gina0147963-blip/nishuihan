...
function exportLineupImage(){
  ...
  // 候補名單（顯示在圖片最底部，比照隊員卡片樣式呈現：職業色底、姓名、職業、已指派技能）
  const reserveNames=(ev.teams.reserve||[]).filter(Boolean);
  const RESERVE_HD=32, RESERVE_COLS=6;
  ...
  for(let r=0;r<reserveRowCount;r++){
    ...
    reserveRowH.push(Math.max(46, 36+maxLines*11+8));
  }
  ...
  if(reserveNames.length){
    ...
    for(let r=0;r<reserveRowCount;r++){
      ...
      for(let c=0;c<RESERVE_COLS;c++){
        ...
        const job=m?jobById(m.jobId):{name:'⚠️查無資料',color:'#e05252',id:''};
        ctx.fillStyle=job.color;
        ctx.fillRect(x,y,SLOTW-6,rowH-4);
        ctx.fillStyle='#fff'; ctx.font='bold 12px sans-serif';
        const label=name.length>10?name.slice(0,10)+'…':name;
        ctx.fillText(label, x+6, y+16);
        ctx.font='10px sans-serif';
        ctx.fillText(job.name, x+6, y+29);
        let ly=y+41;
        slotSkillLines(name).forEach(ln=>{ ctx.font='9px sans-serif'; ctx.fillText(ln, x+6, ly); ly+=11; });
        x+=SLOTW;
      }
      y+=rowH+8;
    }
