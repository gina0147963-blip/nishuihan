...
function playerCardHTML(name,members,teamIdx,key,adminMode,role,isAbsent){
  ...
  return `<div class="pcard jbg-${job.id}${roleCls}${absentCls}"
    ...
    <div class="pjob" style="color:${job.color}">${job.name}</div>
    ${roleLabel}
    ${(function(){const ev=S.events().find(e=>e.id===_curEventId);const as=ev&&ev.assignedSkills&&ev.assignedSkills[name];return as?`<div class="passign">🎴${as}</div>`:'';})()}
    ${isAbsent?'<div class="prole" style="color:var(--txt3)">🌙請假</div>':''}
    ${adminMode&&key!=='reserve'?`<button class="prolebtn" onclick="openRoleModal('${esc}')">⭐</button><button class="prolebtn" style="right:26px" onclick="openAssignSkill('${esc}')" title="指派技能">🎴</button>`:''}
    ...
    for(let i=0;i<SQUAD_COUNT;i++){
      ...
      for(let s=0;s<SQUAD_SIZE;s++){
        ...
        if(name){
          ...
          const asg=(ev.assignedSkills&&ev.assignedSkills[name])||'';
          ctx.fillText(job.name+(asg?' 🎴'+asg:''), x+6, y+37);
        } else {
...
// ============================================================
// 指派技能給玩家（從該玩家已有技能 + 技能清單中選）
// ============================================================
let _assignSkillName=null;
function openAssignSkill(name){
  _assignSkillName=name;
  const ev=S.events().find(e=>e.id===_curEventId);
  const cur=(ev&&ev.assignedSkills&&ev.assignedSkills[name])||'';
  const mem=S.members().find(m=>m.name===name);
  // 優先顯示該玩家已滿等絕技，其次全部絕技清單
  const own=(mem&&mem.skills)||[];
  const all=S.skillList();
  const list=[...own,...all.filter(s=>!own.includes(s))];
  document.getElementById('assign-skill-name').textContent=name;
  document.getElementById('assign-skill-list').innerHTML=list.map(s=>
    `<span class="skill-chip ${cur===s?'active':''} ${own.includes(s)?'own':''}" onclick="pickAssignSkill('${s.replace(/'/g,"\\'")}')">${s}${own.includes(s)?' ⭐':''}</span>`
  ).join('')||'<span class="hint">此玩家尚未設定技能，且技能清單為空</span>';
  openModal('modal-assign-skill');
}
function pickAssignSkill(s){
  const ev=S.events().find(e=>e.id===_curEventId); if(!ev) return;
  if(!ev.assignedSkills) ev.assignedSkills={};
  if(ev.assignedSkills[_assignSkillName]===s) delete ev.assignedSkills[_assignSkillName];
  else ev.assignedSkills[_assignSkillName]=s;
