...
function _buildPlayerBoardHTML(ev){
  ...
  // 找出玩家自己被排在哪個團／小隊（含候補）；找不到就代表這場還沒被排入
  let myTi=null, myKey=null, myIsReserve=false;
  if((ev.teams.reserve||[]).includes(CUR_USER)){
    ...
    for(let ti=0; ti<ev.teamNames.length; ti++){
      for(let i=0;i<SQUAD_COUNT;i++){
        const k=sqKey(ti,i);
        if((ev.teams[k]||[]).includes(CUR_USER)){ myTi=ti; myKey=k; break outer; }
      }
    }
  }
  ...
  if(myIsReserve){
    ...
    const names=ev.teams[myKey]||[];
    const isGuard=(ev.squadRoles||{})[myKey]==='guard';
    const sqIdx=+myKey.match(/_sq(\d+)$/)[1];
    boardHTML=`<div class="team-block">
      <div class="team-hd"><div class="team-label"><span class="team-badge ${cls}">${tName}</span></div></div>
      <div class="squads-grid"><div class="squad-block${isGuard?' squad-guard':''}">
        <div class="squad-hd"><span class="squad-label">${tName}-${sqIdx+1}隊${isGuard?' 🛡️':''}</span><span class="squad-cnt">${names.filter(Boolean).length}/${SQUAD_SIZE}</span></div>
        <div class="squad-slots">${Array.from({length:SQUAD_SIZE},(_,s)=>{ const n=names[s]; return n?playerCardHTML(n,members,myTi,myKey,false,roles[n],false,ev):'<div class="slot-empty"><span class="slot-empty-txt">空</span></div>'; }).join('')}</div>
      </div></div>
    </div>`;
  }
