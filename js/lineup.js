...
function playerCardHTML(name,members,teamIdx,key,adminMode,role,isAbsent){
  ...
  return `<div class="pcard jbg-${job.id}${roleCls}${absentCls}"
    ...
    ${(function(){
      ...
      let html='';
      if(as.length) html+=`<div class="passign">🎴${as.join('、')}</div>`;
      if(ab.length) html+=`<div class="passign passign-b">🀄${ab.join('、')}</div>`;
      return html;
    })()}
...
function exportLineupImage(){
  ...
  function slotSkillLines(name){
    ...
    if(as.length) lines.push(..._wrapCanvasText(mctx,'🎴'+as.join('、'),SLOTW-14));
    if(ab.length) lines.push(..._wrapCanvasText(mctx,'🀄'+ab.join('、'),SLOTW-14));
    return lines;
  }
  ...
  ev.teamNames.forEach((tName,ti)=>{
    ...
    for(let i=0;i<SQUAD_COUNT;i++){
      ...
      ctx.fillText(tName+'-'+(i+1)+'隊'+(isGuard?' 🛡️':''),PAD+8,y+16);
      let x=SLOTX0;
      for(let s=0;s<SQUAD_SIZE;s++){
        const name=names[s];
        const w=SLOTW;
...
function openAssignSkill(name){
  ...
  // 優先顯示該玩家已設定的絕技／群俠技能，其次才是清單其他項目
  const ownS=(mem&&mem.skills)||[];
  const ownB=(mem&&mem.baijia)||[];
  const allS=S.skillList();
  const allB=S.baijiaList();
  const optS=[...ownS,...allS.filter(s=>!ownS.includes(s))];
  const optB=[...ownB,...allB.filter(s=>!ownB.includes(s))];
