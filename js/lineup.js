...
function exportLineupImage(){
  ...
  function teamRowHeights(ti){
    ...
    for(let r=0;r<SQUAD_SIZE;r++){
      ...
      hs.push(Math.max(56, 46+maxLines*16+8));
    }
    return hs;
  }
  ...
  for(let r=0;r<reserveRowCount;r++){
    ...
    reserveRowH.push(Math.max(56, 46+maxLines*16+8));
  }
    ...
    for(let r=0;r<SQUAD_SIZE;r++){
      ...
      for(let i=0;i<SQUAD_COUNT;i++){
        ...
        if(name){
          ...
          ctx.fillText(label.length>11?label.slice(0,11)+'…':label, x+8, y+22);
          ctx.font='13px sans-serif';
          ctx.fillText(job.name, x+8, y+40);
          let ly=y+56;
          slotSkillLines(name).forEach(ln=>{ ctx.font='12px sans-serif'; ctx.fillText(ln, x+8, ly); ly+=16; });
        } else {
          ctx.strokeStyle='#2d3651';
          ctx.strokeRect(x,y+2,COLW-4,rowH-4);
          ctx.fillStyle='#4a5578'; ctx.font='13px sans-serif';
          ctx.fillText('空',x+COLW/2-8,y+rowH/2+4);
        }
        if(isGuard){ // 保鏢小隊整欄描金邊
          ctx.strokeStyle='#f0b843'; ctx.lineWidth=2;
          ctx.strokeRect(x+1,y+1,COLW-6,rowH-2);
          ctx.lineWidth=1;
        }
      }
      y+=rowH;
  ...
  function drawReserve(ctx,y){
    ...
    ctx.fillText('📋 候補（'+reserveNames.length+'人）', PAD+12, y+24);
    y+=RESERVE_HD+8;
    for(let r=0;r<reserveRowCount;r++){
      ...
      for(let c=0;c<RESERVE_COLS;c++){
        ...
        ctx.fillStyle='#fff'; ctx.font='bold 16px sans-serif';
        ctx.fillText(name.length>11?name.slice(0,11)+'…':name, x+8, y+20);
        ctx.font='13px sans-serif';
        ctx.fillText(job.name, x+8, y+38);
        let ly=y+54;
        slotSkillLines(name).forEach(ln=>{ ctx.font='12px sans-serif'; ctx.fillText(ln, x+8, ly); ly+=16; });
      }
      y+=rowH+8;
