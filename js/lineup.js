...
function createEvent(){
  ...
  if(!ok){
    // S.setEvents already shows an error toast via the Store layer; stop here so we don't claim success
    return;
  }
  _curEventId=ev.id;
  ...
  // 不論管理員當下在哪個分頁開啟「新增場次」，建立後一律切到「排表」分頁，
  // 確保使用者能立刻看到新場次已經出現，避免誤以為按了沒反應
  switchTab('a-lineup');
}
function importCore(){
  if(!_curEventId)return;
