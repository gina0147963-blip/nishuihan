...
// 活動日期超過一週視為已過期，排表鎖定不可再編輯（避免誤改舊紀錄）
function _isLineupLocked(evId){
  ...
  const unlockUntil=evDay.getTime()+7*24*60*60*1000;
  return Date.now()>unlockUntil;
}
// 在會修改排表內容的函式最前面呼叫；鎖定時彈出提示並回傳 true 讓呼叫端提早結束
function _guardLineupLock(){
  if(_isLineupLocked(_curEventId)){ toast('此場次活動已超過一週，排表已鎖定，僅供檢視','err'); return true; }
  return false;
}
  ...
  pane.innerHTML=`
    <div class="sec-head">
      ...
      <div class="sec-actions">
        ...
        ${curEv?`
          <button class="btn btn-outline sm" onclick="importCore()" ${locked?'disabled':''}>匯入固定團</button>
    ...
    ${locked?'<p class="hint" style="color:var(--bad)">🔒 此場次活動日期已超過一週，排表已鎖定僅供檢視，無法再編輯（匯出圖片不受影響）</p>':'<p class="hint">💡 場次由「報名管理」建立。左側玩家池分已報名/未報名/請假，拖曳玩家卡到右側格子排表。點卡片可看技能。</p>'}
...
function exportLineupImage(){
  ...
  const RESERVE_HD=32, RESERVE_LINE_H=20;
  mctx.font='13px sans-serif';
  const reserveLines=reserveNames.length?_wrapCanvasText(mctx,reserveNames.join('、'),CW-PAD*2-20):[];
  const reserveH=reserveNames.length?(RESERVE_HD+10+reserveLines.length*RESERVE_LINE_H+16):0;
  ...
  if(reserveNames.length){
    ...
    ctx.fillText('📋 候補（'+reserveNames.length+'人）', PAD+12, y+22);
    y+=RESERVE_HD+8;
    const bodyH=reserveLines.length*RESERVE_LINE_H+16;
    ctx.fillStyle='#1e2538';
    ctx.fillRect(PAD,y,CW-PAD*2,bodyH);
    ctx.fillStyle='#c7ccdb'; ctx.font='13px sans-serif';
    let ly=y+22;
    reserveLines.forEach(ln=>{ ctx.fillText(ln, PAD+10, ly); ly+=RESERVE_LINE_H; });
    y+=bodyH+8;
  }
...
function buildPoolGroups(ev,locked){
  ...
  // 固定團成員排到各分類的最上面，方便管理員優先辨識/拖曳
  const coreFirst=(a,b)=>(b.status==='固定團'?1:0)-(a.status==='固定團'?1:0);
  attendNames.sort(coreFirst); pendingNames.sort(coreFirst); reserveNames.sort(coreFirst); absentNames.sort(coreFirst);
