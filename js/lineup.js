...
function exportLineupImage(){
  ...
  const squadRowH=ev.teamNames.map((tName,ti)=>{
    ...
    for(let i=0;i<SQUAD_COUNT;i++){
      ...
      rowHs.push(Math.max(48, 36+maxLines*15+8));
    }
    return rowHs;
  });
  ...
  for(let r=0;r<reserveRowCount;r++){
    ...
    reserveRowH.push(Math.max(48, 36+maxLines*15+8));
  }
  ...
  ctx.fillText('⚔️ '+ev.name+' '+MODE_LABEL[CUR_MODE]+'（'+(ev.type||'')+'）', PAD, 44);
    ...
    for(let i=0;i<SQUAD_COUNT;i++){
      ...
      for(let s=0;s<SQUAD_SIZE;s++){
        ...
        if(name){
          ...
          ctx.fillStyle='#4a5578'; ctx.font='12px sans-serif';
  ...
  if(reserveNames.length){
    ...
    for(let r=0;r<reserveRowCount;r++){
      ...
      for(let c=0;c<RESERVE_COLS;c++){
        ...
        let ly=y+41;
        slotSkillLines(name).forEach(ln=>{ ctx.font='12px sans-serif'; ctx.fillText(ln, x+6, ly); ly+=15; });
        x+=SLOTW;
      }
      y+=rowH+8;
    }
