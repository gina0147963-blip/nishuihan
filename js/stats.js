...
function renderAdminStats(pane){
  ...
  <div class="stats-grid">
    ...
    <div class="stat-card wide"><h3>📋 預定排表 vs 實際出席（出席率）</h3><div id="plan-vs-actual"></div></div>
    <div class="stat-card wide"><h3>各職業最高單場數據</h3><div id="top-stats"></div></div>
  </div>`;
  setTimeout(()=>{ renderWR(); renderCls(); renderAtt(); renderPlanVsActual(); renderTopStats(); },50);
}
function renderWR(){
  const m=S.matches();
...
// ============================================================
// 預定排表 vs 實際比賽出席率
// 比對每場活動儲存的 plannedRoster 與比賽紀錄 participants
// ============================================================
function renderPlanVsActual(){
  const el=document.getElementById('plan-vs-actual'); if(!el)return;
  const events=S.events().filter(e=>Array.isArray(e.plannedRoster)&&e.plannedRoster.length);
  const matches=S.matches();
  if(!events.length){ el.innerHTML='<p style="color:var(--txt3);font-size:12px">尚無已儲存的排表方案（請在排表頁點「儲存排表方案」）</p>'; return; }

  // 統計每位玩家：預定次數 / 實際出現次數
  const stat={};
  events.forEach(ev=>{
    // 找同日期的比賽紀錄（可能多場）
    const dayMatches=matches.filter(m=>m.date===ev.date);
    const actualSet=new Set();
    dayMatches.forEach(m=>(m.participants||[]).forEach(n=>actualSet.add(n)));
    ev.plannedRoster.forEach(n=>{
      if(!stat[n]) stat[n]={plan:0,act:0};
      stat[n].plan++;
      if(actualSet.size&&actualSet.has(n)) stat[n].act++;
    });
  });

  const rows=Object.entries(stat)
    .map(([n,s])=>({name:n,plan:s.plan,act:s.act,rate:s.plan?Math.round(s.act/s.plan*100):0}))
  ...
  el.innerHTML=rows.map(r=>{
    ...
    return `<div class="att-row">
      ...
      <div class="att-bar-bg"><div class="att-bar" style="width:${r.rate}%;background:${color}"></div></div>
      <span class="att-cnt" style="color:${color}">${r.act}/${r.plan}（${r.rate}%）</span>
    </div>`;
  }).join('')||'<p style="color:var(--txt3);font-size:12px">無資料</p>';
}
