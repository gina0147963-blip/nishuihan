...
function renderAttendance(){
  ...
  box.innerHTML=rows.length?`<div class="att-grid">${rows.map(r=>{
      const color=r.rate>=80?'var(--ok)':r.rate>=50?'var(--gold)':'var(--bad)';
      const job=JOBS.find(j=>j.id===r.jobId);
      return `<div class="att-cell"><span class="att-dot" style="background:${job?job.color:'#888'}"></span><span class="att-nm">${r.name}</span><span class="att-rt" style="color:${color}">${r.rate}%</span><span class="att-sub">${r.act}/${r.plan}</span></div>`;
    }).join('')}</div>`:'<p class="hint">尚無報名資料</p>';
}
