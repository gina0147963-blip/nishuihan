...
function saveData(org, payload) {
  ...
  // ★ 絕技／群俠技能清單安全合併（同一類漏洞的第二處，已一併修正）：
  // 這份清單是「所有組織共用」的一份清單。舊版直接把上傳裝置的本機清單整包覆蓋寫入共用試算表，
  // 只要某台裝置的本機清單比較舊（缺少其他管理員後來新增的技能），同步時就會把那些新增項目
  // 從共用清單洗掉，而且會波及所有幫會/俱樂部。新版改為：讀出雲端既有清單，跟這次上傳的清單
  // 取「聯集」（不是取代），只有明確在刪除紀錄裡的才會被移除，其餘一律保留。
  function _mergeNameList(existingRows, incomingNames, deletedMap) {
    const set = new Set();
    const order = [];
    function add(name){
      if (!name || deletedMap[name]) return;
      if (!set.has(name)) { set.add(name); order.push(name); }
    }
    (existingRows || []).forEach(function(r){ add(r && r.name); });
    (incomingNames || []).forEach(add);
    return order.map(function(name){ return { name: name }; });
  }
  if (writeSheet(shared, '共用_絕技清單',
    _mergeNameList(readSheet(shared, '共用_絕技清單'), payload.skillList, skillDeletedNames), ['name'])) _changed = true;
  if (writeSheet(shared, '共用_群俠技能清單',
    _mergeNameList(readSheet(shared, '共用_群俠技能清單'), payload.baijiaList, skillDeletedNames), ['name'])) _changed = true;
  ...
  // ★★ 報名紀錄逐格精準合併（比上一版的「逐場次合併」更安全一層）★★
  // 上一版改成「只覆蓋前端有帶到的場次」，但仍有殘留風險：如果某台裝置的本機報名快取
  // 剛好只是「不完整」（例如報名還沒完全同步就被使用者觸發寫入），它的本機版本對某場次
  // 而言依然「存在」（只是缺人），一樣會被誤判為「這次有帶到」而整場覆蓋掉，洗掉真正的報名。
  // 現在改用前端隨附的 signupDirty（本機真正被使用者改過的 evId|姓名 清單）逐格比對：
  // 只有「這一格」被標記為本機真的改過，才會覆蓋雲端那一格；其餘格子完全不動，
  // 不管前端那個場次的整包資料完不完整都不影響——徹底排除整場/整包覆蓋的可能。
  if (payload.signups && typeof payload.signups === 'object') {
    var existingSignupRows = readSheet(ss, prefix + '報名紀錄');
    var cellMap = {}; // 'evId|playerName' -> status
    var order = [];
    existingSignupRows.forEach(function(r){
      if (!r.eventId || !r.playerName) return;
      var k = r.eventId + '|' + r.playerName;
      if (!(k in cellMap)) order.push(k);
      cellMap[k] = r.status;
    });
    var dirty = payload.signupDirty;
    if (dirty && typeof dirty === 'object') {
      // 新版前端：只套用真正標記為本機改過的格子
      Object.keys(dirty).forEach(function(k){
        var sep = k.indexOf('|'); if (sep < 0) return;
        var evId = k.slice(0, sep), name = k.slice(sep + 1);
        var val = (payload.signups[evId] || {})[name];
        if (val === undefined) {
          if (k in cellMap) delete cellMap[k]; // 本機明確取消/清除這筆
        } else {
          if (!(k in cellMap)) order.push(k);
          cellMap[k] = val; // 本機真的改過 → 覆蓋雲端這一格
        }
      });
    } else {
      // 舊版前端相容（尚未更新、沒有 signupDirty）：退回「只覆蓋有帶到的場次」，
      // 仍比最原始版本（整包覆蓋全部）安全很多。
      var touchedEv = {};
      Object.keys(payload.signups).forEach(function(evId){ touchedEv[evId] = true; });
      Object.keys(cellMap).forEach(function(k){
        var evId = k.slice(0, k.indexOf('|'));
        if (touchedEv[evId]) delete cellMap[k];
      });
      Object.keys(payload.signups).forEach(function(evId){
        var map = payload.signups[evId] || {};
    ...
    var finalRows = order.filter(function(k){ return k in cellMap; }).map(function(k){
      var sep = k.indexOf('|');
      return { eventId: k.slice(0, sep), playerName: k.slice(sep + 1), status: cellMap[k] };
    });
    if (writeSheet(ss, prefix + '報名紀錄', finalRows, ['eventId','playerName','status'])) _changed = true;
  }
